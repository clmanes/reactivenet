// GENERATED by scripts/generate-spectrum-registry.mjs — do not edit.
//
// Source: the Custom Elements Manifests shipped by @spectrum-web-components.
// Regenerate with `bun run spectrum:registry` after upgrading the library.
//
// 92 components, 482 attributes.

type kind =
  | Flag
  | Number
  | Choice
  | Text

type attribute = {
  name: string,
  /** From the manifest; empty when the library does not document it. */
  description: string,
  kind: kind,
  choices: array<string>,
  fallback: string,
}

type component = {
  directive: string,
  /** From the manifest; empty when the library does not document it. */
  description: string,
  tag: string,
  package: string,
  slots: array<string>,
  attributes: array<attribute>,
}

let all: array<component> = [
  {
    directive: "accordion",
    description: "",
    tag: "sp-accordion",
    package: "accordion",
    slots: [""],
    attributes: [
      {
        name: "allow-multiple",
        description: "Allows multiple accordion items to be opened at the same time",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "density",
        description: "Sets the spacing between the content to borders of an accordion item",
        kind: Choice,
        choices: ["compact", "spacious"],
        fallback: "",
      },
      {
        name: "level",
        description: "The heading level (2-6) to use for all accordion item titles.",
        kind: Number,
        choices: [],
        fallback: "3",
      }
    ],
  },
  {
    directive: "accordion-item",
    description: "",
    tag: "sp-accordion-item",
    package: "accordion",
    slots: [""],
    attributes: [
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "level",
        description: "The heading level (2-6) to use for the accordion item title.",
        kind: Number,
        choices: [],
        fallback: "3",
      }
    ],
  },
  {
    directive: "action-bar",
    description: "",
    tag: "sp-action-bar",
    package: "action-bar",
    slots: [""],
    attributes: [
      {
        name: "emphasized",
        description: "Deliver the Action Bar with additional visual emphasis.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "flexible",
        description: "When `flexible` the action bar sizes itself to its content",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "variant",
        description: "The variant applies specific styling when set to `sticky` or `fixed`.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "action-button",
    description: "",
    tag: "sp-action-button",
    package: "action-button",
    slots: ["", "icon"],
    attributes: [
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "hold-affordance",
        description: "Hold affordance is not included in the initial 2nd-gen `swc-action-button`",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "quiet",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "button",
      },
      {
        name: "selected",
        description: "Whether an Action Button with `role='button'`",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "toggles",
        description: "Whether to automatically manage the `selected`",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "The static color variant to use for the action button.",
        kind: Choice,
        choices: ["white", "black"],
        fallback: "",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "action-group",
    description: "",
    tag: "sp-action-group",
    package: "action-group",
    slots: [""],
    attributes: [
      {
        name: "compact",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "justified",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "quiet",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selects",
        description: "",
        kind: Choice,
        choices: ["single", "multiple"],
        fallback: "",
      },
      {
        name: "static-color",
        description: "",
        kind: Choice,
        choices: ["white", "black"],
        fallback: "",
      },
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "action-menu",
    description: "An `<sp-action-menu>` is an action button with an attached menu of options.",
    tag: "sp-action-menu",
    package: "action-menu",
    slots: ["", "icon", "label", "label-only", "tooltip"],
    attributes: [
      {
        name: "selects",
        description: "The selection mode for the action menu.",
        kind: Text,
        choices: [],
        fallback: "undefined",
      },
      {
        name: "icons",
        description: "Controls how icons are displayed in the picker button.",
        kind: Choice,
        choices: ["only", "none"],
        fallback: "",
      },
      {
        name: "invalid",
        description: "Whether the picker is in an invalid state. Displays a validation icon when true.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "pending-label",
        description: "Defines a string value that labels the Picker while it is in pending state.",
        kind: Text,
        choices: [],
        fallback: "Pending",
      },
      {
        name: "label",
        description: "The placeholder label displayed when no item is selected.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "Applies static color styling for use on colored backgrounds.",
        kind: Choice,
        choices: ["white", "black"],
        fallback: "",
      },
      {
        name: "quiet",
        description: "Whether to render the picker in quiet mode with minimal visual styling.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "The current value of the picker, corresponding to the selected menu item's value.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "alert-banner",
    description: "",
    tag: "sp-alert-banner",
    package: "alert-banner",
    slots: ["", "action"],
    attributes: [
      {
        name: "open",
        description: "Controls the display of the alert banner.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "dismissible",
        description: "Whether to include an icon-only close button to dismiss the alert banner.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "variant",
        description: "The variant applies specific styling when set to `negative` or `info`;",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "alert-dialog",
    description: "",
    tag: "sp-alert-dialog",
    package: "alert-dialog",
    slots: [],
    attributes: [
      {
        name: "variant",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "asset",
    description: "",
    tag: "sp-asset",
    package: "asset",
    slots: [""],
    attributes: [
      {
        name: "variant",
        description: "The variant of the asset. When not provided, slot content is rendered (e.g., an image).",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "Accessible label for the asset's file or folder variant.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "avatar",
    description: "",
    tag: "sp-avatar",
    package: "avatar",
    slots: [],
    attributes: [
      {
        name: "src",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "is-decorative",
        description: "When true, marks the avatar as decorative and hides it from screen readers.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "size",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "badge",
    description: "",
    tag: "sp-badge",
    package: "badge",
    slots: ["", "icon"],
    attributes: [
      {
        name: "variant",
        description: "The variant of the badge.",
        kind: Text,
        choices: [],
        fallback: "informative",
      },
      {
        name: "size",
        description: "The size of the badge.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "fixed",
        description: "The fixed position of the badge.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "breadcrumb-item",
    description: "",
    tag: "sp-breadcrumb-item",
    package: "breadcrumbs",
    slots: [],
    attributes: [
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "undefined",
      },
      {
        name: "isLastOfType",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "breadcrumbs",
    description: "",
    tag: "sp-breadcrumbs",
    package: "breadcrumbs",
    slots: ["icon", "root", ""],
    attributes: [
      {
        name: "max-visible-items",
        description: "Override the maximum number of visible items",
        kind: Number,
        choices: [],
        fallback: "4",
      },
      {
        name: "label",
        description: "Accessible name for the Breadcrumbs component",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "menu-label",
        description: "Change the default label of the action menu",
        kind: Text,
        choices: [],
        fallback: "More items",
      },
      {
        name: "compact",
        description: "compact option is useful for reducing the height of the breadcrumbs",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "button",
    description: "",
    tag: "sp-button",
    package: "button",
    slots: ["", "icon"],
    attributes: [
      {
        name: "pending-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Pending",
      },
      {
        name: "pending",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "variant",
        description: "The visual variant to apply to this button.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "The static color variant to use for this button.",
        kind: Choice,
        choices: ["black", "white"],
        fallback: "",
      },
      {
        name: "treatment",
        description: "The visual treatment to apply to this button.",
        kind: Text,
        choices: [],
        fallback: "fill",
      },
      {
        name: "quiet",
        description: "Style this button to be less obvious.",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "no-wrap",
        description: "Disables text wrapping within the button component's label.",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "active",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "type",
        description: "The default behavior of the button.",
        kind: Choice,
        choices: ["button", "submit", "reset"],
        fallback: "button",
      }
    ],
  },
  {
    directive: "button-group",
    description: "",
    tag: "sp-button-group",
    package: "button-group",
    slots: [""],
    attributes: [
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "card",
    description: "",
    tag: "sp-card",
    package: "card",
    slots: ["preview", "cover-photo", "heading", "subheading", "description", "actions", "footer"],
    attributes: [
      {
        name: "asset",
        description: "",
        kind: Choice,
        choices: ["file", "folder"],
        fallback: "",
      },
      {
        name: "variant",
        description: "",
        kind: Choice,
        choices: ["standard", "gallery", "quiet"],
        fallback: "standard",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "heading",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "horizontal",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "toggles",
        description: "Indicates whether the card can be toggled between selected and unselected states.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "subheading",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "checkbox",
    description: "",
    tag: "sp-checkbox",
    package: "checkbox",
    slots: [""],
    attributes: [
      {
        name: "disabled",
        description: "Disable this control. It will not receive focus or events",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "indeterminate",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "invalid",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "tabindex",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "checked",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "name",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "readonly",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "clear-button",
    description: "",
    tag: "sp-clear-button",
    package: "button",
    slots: ["", "icon"],
    attributes: [
      {
        name: "label",
        description: "An accessible label that describes the component.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "quiet",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "variant",
        description: "The visual variant to apply to this button.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "The visual variant to apply to this button.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "active",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "type",
        description: "The default behavior of the button.",
        kind: Choice,
        choices: ["button", "submit", "reset"],
        fallback: "button",
      }
    ],
  },
  {
    directive: "close-button",
    description: "",
    tag: "sp-close-button",
    package: "button",
    slots: ["", "icon"],
    attributes: [
      {
        name: "variant",
        description: "The visual variant to apply to this button.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "",
        kind: Choice,
        choices: ["black", "white"],
        fallback: "",
      },
      {
        name: "active",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "type",
        description: "The default behavior of the button.",
        kind: Choice,
        choices: ["button", "submit", "reset"],
        fallback: "button",
      }
    ],
  },
  {
    directive: "coach-indicator",
    description: "",
    tag: "sp-coach-indicator",
    package: "coachmark",
    slots: [],
    attributes: [
      {
        name: "quiet",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "static-color",
        description: "",
        kind: Choice,
        choices: ["white", "black"],
        fallback: "",
      }
    ],
  },
  {
    directive: "coachmark",
    description: "",
    tag: "sp-coachmark",
    package: "coachmark",
    slots: ["cover-photo", "heading", "description", "actions", "step-count"],
    attributes: [
      {
        name: "item",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "right",
      },
      {
        name: "shortcut-key",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "modifierKeys",
        description: "",
        kind: Text,
        choices: [],
        fallback: "[]",
      },
      {
        name: "src",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "media-type",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "has-asset",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "asset",
        description: "",
        kind: Choice,
        choices: ["file", "folder"],
        fallback: "",
      },
      {
        name: "current-step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "total-steps",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "primary-cta",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "secondary-cta",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "color-area",
    description: "",
    tag: "sp-color-area",
    package: "color-area",
    slots: ["gradient"],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label-x",
        description: "",
        kind: Text,
        choices: [],
        fallback: "saturation",
      },
      {
        name: "label-y",
        description: "",
        kind: Text,
        choices: [],
        fallback: "luminosity",
      },
      {
        name: "hue",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "x",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "y",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0.01",
      }
    ],
  },
  {
    directive: "color-field",
    description: "",
    tag: "sp-color-field",
    package: "color-field",
    slots: [],
    attributes: [
      {
        name: "view-color",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "color-handle",
    description: "",
    tag: "sp-color-handle",
    package: "color-handle",
    slots: [],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "rgba(255, 0, 0, 0.5)",
      }
    ],
  },
  {
    directive: "color-loupe",
    description: "",
    tag: "sp-color-loupe",
    package: "color-loupe",
    slots: [],
    attributes: [
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "rgba(255, 0, 0, 0.5)",
      }
    ],
  },
  {
    directive: "color-slider",
    description: "",
    tag: "sp-color-slider",
    package: "color-slider",
    slots: ["gradient"],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "hue",
      },
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "1",
      }
    ],
  },
  {
    directive: "color-wheel",
    description: "",
    tag: "sp-color-wheel",
    package: "color-wheel",
    slots: ["gradient"],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "hue",
      },
      {
        name: "step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "1",
      },
      {
        name: "value",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "combobox",
    description: "",
    tag: "sp-combobox",
    package: "combobox",
    slots: ["", "tooltip"],
    attributes: [
      {
        name: "autocomplete",
        description: "",
        kind: Choice,
        choices: ["list", "none"],
        fallback: "none",
      },
      {
        name: "open",
        description: "Whether the listbox is visible.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "pending",
        description: "Whether the items are currently loading.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "pending-label",
        description: "Defines a string value that labels the Combobox while it is in pending state.",
        kind: Text,
        choices: [],
        fallback: "Pending",
      },
      {
        name: "options",
        description: "An array of options to present in the Menu provided while typing into the input",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "contextual-help",
    description: "Spectrum Contextual help provides additional information about",
    tag: "sp-contextual-help",
    package: "contextual-help",
    slots: ["heading", "Text", "link"],
    attributes: [
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "bottom-start",
      },
      {
        name: "label",
        description: "Provides an accessible name for the action button trigger.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "variant",
        description: "The `variant` property applies specific styling on the action button trigger.",
        kind: Choice,
        choices: ["info", "help"],
        fallback: "info",
      },
      {
        name: "offset",
        description: "The `offset` property accepts either a single number, to",
        kind: Text,
        choices: [],
        fallback: "0",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "dialog",
    description: "",
    tag: "sp-dialog",
    package: "dialog",
    slots: ["hero", "heading", "", "footer", "button"],
    attributes: [
      {
        name: "error",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "dismissable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "dismiss-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Close",
      },
      {
        name: "no-divider",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mode",
        description: "",
        kind: Choice,
        choices: ["fullscreen", "fullscreenTakeover"],
        fallback: "",
      },
      {
        name: "size",
        description: "",
        kind: Choice,
        choices: ["s", "m", "l"],
        fallback: "",
      }
    ],
  },
  {
    directive: "dialog-base",
    description: "",
    tag: "sp-dialog-base",
    package: "dialog",
    slots: [""],
    attributes: [
      {
        name: "dismissable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mode",
        description: "",
        kind: Choice,
        choices: ["fullscreen", "fullscreenTakeover"],
        fallback: "",
      },
      {
        name: "responsive",
        description: "When set to true, fills screens smaller than 350px high and 400px wide with the full dialog.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "underlay",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "dialog-wrapper",
    description: "",
    tag: "sp-dialog-wrapper",
    package: "dialog",
    slots: [""],
    attributes: [
      {
        name: "error",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "cancel-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "confirm-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "dismiss-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Close",
      },
      {
        name: "footer",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "hero",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "hero-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "no-divider",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "size",
        description: "",
        kind: Choice,
        choices: ["s", "m", "l"],
        fallback: "",
      },
      {
        name: "secondary-label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "headline",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "headline-visibility",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "dismissable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mode",
        description: "",
        kind: Choice,
        choices: ["fullscreen", "fullscreenTakeover"],
        fallback: "",
      },
      {
        name: "responsive",
        description: "When set to true, fills screens smaller than 350px high and 400px wide with the full dialog.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "underlay",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "divider",
    description: "",
    tag: "sp-divider",
    package: "divider",
    slots: [],
    attributes: [
      {
        name: "size",
        description: "The size of the divider.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "vertical",
        description: "Whether the divider is vertical. If false, the divider is horizontal. The default is false.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "static-color",
        description: "The static color variant to use for the divider.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "dropzone",
    description: "",
    tag: "sp-dropzone",
    package: "dropzone",
    slots: [""],
    attributes: [
      {
        name: "dropEffect",
        description: "Controls the feedback (typically visual) the user is given during a drag and drop operation",
        kind: Choice,
        choices: ["copy", "move", "link", "none"],
        fallback: "",
      },
      {
        name: "dragged",
        description: "Indicates that files are currently being dragged over the dropzone.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "filled",
        description: "Set this property to indicate that the component is in a filled state.",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "field-group",
    description: "",
    tag: "sp-field-group",
    package: "field-group",
    slots: ["", "help-text", "negative-help-text"],
    attributes: [
      {
        name: "horizontal",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "invalid",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "field-label",
    description: "",
    tag: "sp-field-label",
    package: "field-label",
    slots: [""],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "id",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "for",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "required",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "side-aligned",
        description: "",
        kind: Choice,
        choices: ["start", "end"],
        fallback: "",
      }
    ],
  },
  {
    directive: "grid",
    description: "",
    tag: "sp-grid",
    package: "grid",
    slots: [],
    attributes: [
      {
        name: "focusableSelector",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "gap",
        description: "",
        kind: Text,
        choices: [],
        fallback: "0",
      },
      {
        name: "padding",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "items",
        description: "",
        kind: Text,
        choices: [],
        fallback: "[]",
      },
      {
        name: "itemSize",
        description: "",
        kind: Text,
        choices: [],
        fallback: "{ width: 200, height: 200, }",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "[]",
      }
    ],
  },
  {
    directive: "help-text",
    description: "",
    tag: "sp-help-text",
    package: "help-text",
    slots: [],
    attributes: [
      {
        name: "icon",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "variant",
        description: "The visual variant to apply to this help text.",
        kind: Text,
        choices: [],
        fallback: "neutral",
      }
    ],
  },
  {
    directive: "icon",
    description: "",
    tag: "sp-icon",
    package: "icon",
    slots: [],
    attributes: [
      {
        name: "src",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "name",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "size",
        description: "",
        kind: Choice,
        choices: ["xxs", "xs", "s", "m", "l", "xl", "xxl"],
        fallback: "",
      }
    ],
  },
  {
    directive: "illustrated-message",
    description: "",
    tag: "sp-illustrated-message",
    package: "illustrated-message",
    slots: ["", "heading", "description"],
    attributes: [
      {
        name: "heading",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "description",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "infield-button",
    description: "",
    tag: "sp-infield-button",
    package: "infield-button",
    slots: [],
    attributes: [
      {
        name: "block",
        description: "Whether to style the button as if it is at the start or end of a vertical stack",
        kind: Choice,
        choices: ["start", "end"],
        fallback: "",
      },
      {
        name: "inline",
        description: "Whether to style the button as if it is at the start or end of a horizontal group",
        kind: Choice,
        choices: ["start", "end"],
        fallback: "",
      },
      {
        name: "quiet",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "link",
    description: "",
    tag: "sp-link",
    package: "link",
    slots: [],
    attributes: [
      {
        name: "variant",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "static-color",
        description: "",
        kind: Choice,
        choices: ["black", "white"],
        fallback: "",
      },
      {
        name: "quiet",
        description: "Uses quiet styles or not",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "menu",
    description: "Spectrum Menu Component",
    tag: "sp-menu",
    package: "menu",
    slots: [""],
    attributes: [
      {
        name: "selects",
        description: "how the menu allows selection of its items:",
        kind: Choice,
        choices: ["inherit", "single", "multiple"],
        fallback: "",
      },
      {
        name: "label",
        description: "label of the menu",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "ignore",
        description: "whether menu should be ignored by roving tabindex controller",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mobile-view",
        description: "Enables mobile submenu navigation where tapping a submenu item replaces",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mobile-back-label",
        description: "Label for the mobile back button, used for localization.",
        kind: Text,
        choices: [],
        fallback: "Back",
      },
      {
        name: "value",
        description: "value of the selected item(s)",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "value-separator",
        description: "",
        kind: Text,
        choices: [],
        fallback: ",",
      }
    ],
  },
  {
    directive: "menu-divider",
    description: "",
    tag: "sp-menu-divider",
    package: "menu",
    slots: [],
    attributes: [

    ],
  },
  {
    directive: "menu-group",
    description: "",
    tag: "sp-menu-group",
    package: "menu",
    slots: ["header", ""],
    attributes: [
      {
        name: "selects",
        description: "how the menu allows selection of its items:",
        kind: Choice,
        choices: ["inherit", "single", "multiple"],
        fallback: "",
      },
      {
        name: "label",
        description: "label of the menu",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "ignore",
        description: "whether menu should be ignored by roving tabindex controller",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mobile-view",
        description: "Enables mobile submenu navigation where tapping a submenu item replaces",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "mobile-back-label",
        description: "Label for the mobile back button, used for localization.",
        kind: Text,
        choices: [],
        fallback: "Back",
      },
      {
        name: "value",
        description: "value of the selected item(s)",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "value-separator",
        description: "",
        kind: Text,
        choices: [],
        fallback: ",",
      }
    ],
  },
  {
    directive: "menu-item",
    description: "",
    tag: "sp-menu-item",
    package: "menu",
    slots: ["", "description", "icon", "value", "submenu"],
    attributes: [
      {
        name: "active",
        description: "whether the menu item is active or has an active descendant",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "whether the menu item has keyboard focus",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selected",
        description: "whether the menu item is selected",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "value of the menu item which is used for selection",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "has-submenu",
        description: "whether the menu item has a submenu",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "no-wrap",
        description: "whether menu item text content should not wrap",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "whether submenu is open",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "meter",
    description: "",
    tag: "sp-meter",
    package: "meter",
    slots: [""],
    attributes: [
      {
        name: "progress",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "variant",
        description: "The variant applies specific styling when set to `negative`, `positive`, `notice`",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "side-label",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "static-color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "number-field",
    description: "",
    tag: "sp-number-field",
    package: "number-field",
    slots: ["help-text", "negative-help-text"],
    attributes: [
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "format-options",
        description: "An `&lt;sp-number-field&gt;` element will process its numeric value with",
        kind: Text,
        choices: [],
        fallback: "{}",
      },
      {
        name: "hide-stepper",
        description: "Whether the stepper UI is hidden or not.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "indeterminate",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "keyboard-focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "max",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "min",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "step",
        description: "The distance by which to alter the value of the element when taking a \"step\".",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "step-modifier",
        description: "",
        kind: Number,
        choices: [],
        fallback: "10",
      },
      {
        name: "value",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "overlay",
    description: "",
    tag: "sp-overlay",
    package: "overlay",
    slots: ["default"],
    attributes: [
      {
        name: "placement",
        description: "Instruct the Overlay where to place itself in relationship to the trigger element.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "offset",
        description: "The `offset` property accepts either a single number to define the offset of the",
        kind: Text,
        choices: [],
        fallback: "0",
      },
      {
        name: "disabled",
        description: "Indicates whether the overlay is currently functional or not.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "receives-focus",
        description: "Whether to pass focus to the overlay once opened, or",
        kind: Choice,
        choices: ["true", "false", "auto"],
        fallback: "auto",
      },
      {
        name: "delayed",
        description: "An Overlay that is `delayed` will wait until a warm-up period of 1000ms",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "Indicates whether the Overlay is projected onto the \"top layer\" or not.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "allow-outside-click",
        description: "DEPRECATED: Whether clicks outside the overlay should close it (not recommended for accessibility)",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "tip-padding",
        description: "The padding around the tip of the overlay.",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "trigger",
        description: "An optional ID reference for the trigger element combined with the optional",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "type",
        description: "Configures the open/close heuristics of the Overlay.",
        kind: Text,
        choices: [],
        fallback: "auto",
      }
    ],
  },
  {
    directive: "overlay-trigger",
    description: "",
    tag: "overlay-trigger",
    package: "overlay",
    slots: ["trigger", "hover-content", "click-content", "longpress-content", "longpress-describedby-descriptor"],
    attributes: [
      {
        name: "placement",
        description: "The placement of the overlay relative to the trigger",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "offset",
        description: "The distance between the overlay and the trigger",
        kind: Number,
        choices: [],
        fallback: "6",
      },
      {
        name: "disabled",
        description: "Whether the overlay trigger is disabled",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "receives-focus",
        description: "How focus should be handled ('true'|'false'|'auto')",
        kind: Choice,
        choices: ["true", "false", "auto"],
        fallback: "auto",
      },
      {
        name: "triggered-by",
        description: "Optional property to optimize performance and prevent race conditions.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "type",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "open",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "picker",
    description: "An `<sp-picker>` is a dropdown selection component that allows users to choose",
    tag: "sp-picker",
    package: "picker",
    slots: ["label", "description", "tooltip", ""],
    attributes: [
      {
        name: "icons",
        description: "Controls how icons are displayed in the picker button.",
        kind: Choice,
        choices: ["only", "none"],
        fallback: "",
      },
      {
        name: "invalid",
        description: "Whether the picker is in an invalid state. Displays a validation icon when true.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "pending-label",
        description: "Defines a string value that labels the Picker while it is in pending state.",
        kind: Text,
        choices: [],
        fallback: "Pending",
      },
      {
        name: "label",
        description: "The placeholder label displayed when no item is selected.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "quiet",
        description: "Whether to render the picker in quiet mode with minimal visual styling.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "The current value of the picker, corresponding to the selected menu item's value.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "placement",
        description: "The preferred placement of the component's overlay relative to the trigger button.",
        kind: Text,
        choices: [],
        fallback: "bottom-start",
      },
      {
        name: "disabled",
        description: "Whether the component is disabled. When disabled, the component cannot be interacted with.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "focused",
        description: "Whether the component currently has visible focus.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "readonly",
        description: "Whether the component is read-only. When read-only, the component displays its value but cannot be changed.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "pending",
        description: "Whether the items are currently loading.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "force-popover",
        description: "Forces the component to render as a popover on mobile instead of a tray.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "open",
        description: "Whether the component's menu overlay is currently open.",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "picker-button",
    description: "",
    tag: "sp-picker-button",
    package: "picker-button",
    slots: [],
    attributes: [
      {
        name: "invalid",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "position",
        description: "",
        kind: Choice,
        choices: ["left", "right"],
        fallback: "right",
      }
    ],
  },
  {
    directive: "popover",
    description: "",
    tag: "sp-popover",
    package: "popover",
    slots: [""],
    attributes: [
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "open",
        description: "Whether the popover is visible or not.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "tip",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "progress-bar",
    description: "",
    tag: "sp-progress-bar",
    package: "progress-bar",
    slots: [],
    attributes: [
      {
        name: "indeterminate",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "over-background",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "side-label",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "progress",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "static-color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "progress-circle",
    description: "",
    tag: "sp-progress-circle",
    package: "progress-circle",
    slots: [""],
    attributes: [
      {
        name: "static-color",
        description: "Static color variant for use on different backgrounds.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "size",
        description: "The size of the progress circle.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "indeterminate",
        description: "Whether the progress circle shows indeterminate progress (loading state).",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "Accessible label for the progress circle.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "progress",
        description: "Progress value from 0 to 100.",
        kind: Number,
        choices: [],
        fallback: "0",
      }
    ],
  },
  {
    directive: "radio",
    description: "",
    tag: "sp-radio",
    package: "radio",
    slots: [""],
    attributes: [
      {
        name: "invalid",
        description: "Uses the invalid style",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "disabled",
        description: "Uses the disabled style",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "checked",
        description: "Represents when the input is checked",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "Identifies this radio button within its radio group",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "autofocus",
        description: "When this control is rendered, focus it automatically",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "readonly",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "radio-group",
    description: "",
    tag: "sp-radio-group",
    package: "radio",
    slots: ["", "help-text", "negative-help-text"],
    attributes: [
      {
        name: "name",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "search",
    description: "",
    tag: "sp-search",
    package: "search",
    slots: ["help-text", "negative-help-text"],
    attributes: [
      {
        name: "action",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Search",
      },
      {
        name: "method",
        description: "",
        kind: Choice,
        choices: ["get", "post", "dialog"],
        fallback: "",
      },
      {
        name: "placeholder",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Search",
      },
      {
        name: "holdValueOnEscape",
        description: "Controls whether the typed value should be held (i.e., not cleared or reset) when the Escape key is pressed.",
        kind: Flag,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "sidenav",
    description: "",
    tag: "sp-sidenav",
    package: "sidenav",
    slots: [""],
    attributes: [
      {
        name: "manage-tab-index",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "undefined",
      },
      {
        name: "variant",
        description: "The multilevel variant will have multiple layers of hierarchical navigation items.",
        kind: Text,
        choices: [],
        fallback: "undefined",
      },
      {
        name: "label",
        description: "An accessible label that describes the component,",
        kind: Text,
        choices: [],
        fallback: "undefined",
      }
    ],
  },
  {
    directive: "sidenav-heading",
    description: "",
    tag: "sp-sidenav-heading",
    package: "sidenav",
    slots: [""],
    attributes: [
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "sidenav-item",
    description: "",
    tag: "sp-sidenav-item",
    package: "sidenav",
    slots: [""],
    attributes: [
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "undefined",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "expanded",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "slider",
    description: "",
    tag: "sp-slider",
    package: "slider",
    slots: ["", "handle"],
    attributes: [
      {
        name: "editable",
        description: "Whether to display a Number Field along side the slider UI",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "hide-stepper",
        description: "Whether the stepper UI of the Number Field is hidden or not",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "type",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "variant",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label-visibility",
        description: "",
        kind: Choice,
        choices: ["text", "value", "none"],
        fallback: "",
      },
      {
        name: "min",
        description: "",
        kind: Text,
        choices: [],
        fallback: "0",
      },
      {
        name: "max",
        description: "",
        kind: Text,
        choices: [],
        fallback: "100",
      },
      {
        name: "step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "1",
      },
      {
        name: "tick-step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "tick-labels",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "fill-start",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "quiet",
        description: "Applies `quiet` to the underlying `sp-number-field` when `editable === true`.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "indeterminate",
        description: "Applies `indeterminate` to the underlying `sp-number-field` when `editable === true`. Is removed on the next `change` event.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "By default, the value of a Slider Handle will be halfway between its",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "default-value",
        description: "Set the default value of the handle. Setting this property will cause the",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "dragging",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "highlight",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "name",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "format-options",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "slider-handle",
    description: "",
    tag: "sp-slider-handle",
    package: "slider",
    slots: [],
    attributes: [
      {
        name: "value",
        description: "By default, the value of a Slider Handle will be halfway between its",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "default-value",
        description: "Set the default value of the handle. Setting this property will cause the",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "dragging",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "highlight",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "name",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "min",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "max",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "step",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "format-options",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "split-view",
    description: "",
    tag: "sp-split-view",
    package: "split-view",
    slots: ["Two"],
    attributes: [
      {
        name: "primarySize",
        description: "The start size of the primary pane, can be a real pixel number|string, percentage or \"auto\"",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "resizable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "collapsible",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "primary-min",
        description: "The minimum size of the primary pane",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "primary-max",
        description: "The maximum size of the primary pane",
        kind: Text,
        choices: [],
        fallback: "DEFAULT_MAX_SIZE",
      },
      {
        name: "primary-size",
        description: "The start size of the primary pane, can be a real pixel number|string, percentage or \"auto\"",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "secondary-min",
        description: "The minimum size of the secondary pane",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "secondary-max",
        description: "The maximum size of the secondary pane",
        kind: Text,
        choices: [],
        fallback: "DEFAULT_MAX_SIZE",
      },
      {
        name: "splitter-pos",
        description: "The current splitter position of split-view",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "Sets the `aria-label` on the splitter component",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "viewSize",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      }
    ],
  },
  {
    directive: "status-light",
    description: "",
    tag: "sp-status-light",
    package: "status-light",
    slots: [""],
    attributes: [
      {
        name: "variant",
        description: "The variant of the status light.",
        kind: Text,
        choices: [],
        fallback: "info",
      },
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "size",
        description: "The size of the status light.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "swatch",
    description: "",
    tag: "sp-swatch",
    package: "swatch",
    slots: [],
    attributes: [
      {
        name: "border",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "color",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "mixed-value",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "nothing",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "button",
      },
      {
        name: "rounding",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "shape",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "swatch-group",
    description: "",
    tag: "sp-swatch-group",
    package: "swatch",
    slots: [""],
    attributes: [
      {
        name: "border",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "density",
        description: "",
        kind: Choice,
        choices: ["compact", "spacious"],
        fallback: "",
      },
      {
        name: "rounding",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selects",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "shape",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "switch",
    description: "",
    tag: "sp-switch",
    package: "switch",
    slots: [""],
    attributes: [
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "tab",
    description: "",
    tag: "sp-tab",
    package: "tabs",
    slots: ["", "icon"],
    attributes: [
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "vertical",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "tab-panel",
    description: "",
    tag: "sp-tab-panel",
    package: "tabs",
    slots: [""],
    attributes: [
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "table",
    description: "",
    tag: "sp-table",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "grid",
      },
      {
        name: "selects",
        description: "Whether the Table allows users to select a row or rows, and thus controls whether or not the Table also renders checkboxes.",
        kind: Choice,
        choices: ["single", "multiple"],
        fallback: "",
      },
      {
        name: "selected",
        description: "An array of <sp-row> values that have been selected.",
        kind: Text,
        choices: [],
        fallback: "[]",
      },
      {
        name: "select-all-label",
        description: "The accessible label for the \"select all\" checkbox in the table header.",
        kind: Text,
        choices: [],
        fallback: "Select All",
      },
      {
        name: "items",
        description: "The content of the rows rendered by the virtualized table. The key is the value of the sp-table-row, and the value is the sp-table-row's content (not the row itself).",
        kind: Text,
        choices: [],
        fallback: "[]",
      },
      {
        name: "itemValue",
        description: "The value of an item. By default, it is set to the index of the sp-table-row.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "itemLabel",
        description: "A function to extract the accessible label for a row's checkbox from an item.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "scroller",
        description: "Whether or not the virtualized table has a scroll bar. If this is set to true, make sure to specify a height in the sp-table's inline styles.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "emphasized",
        description: "Deliver the Table with additional visual emphasis to selected rows.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "quiet",
        description: "Display with \"quiet\" variant styles.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "density",
        description: "Changes the spacing around table cell content.",
        kind: Choice,
        choices: ["compact", "spacious"],
        fallback: "",
      }
    ],
  },
  {
    directive: "table-body",
    description: "",
    tag: "sp-table-body",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "rowgroup",
      }
    ],
  },
  {
    directive: "table-cell",
    description: "",
    tag: "sp-table-cell",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "gridcell",
      }
    ],
  },
  {
    directive: "table-checkbox-cell",
    description: "",
    tag: "sp-table-checkbox-cell",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "head-cell",
        description: "Whether or not the checkbox cell is in the table head.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "gridcell",
      },
      {
        name: "indeterminate",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "checked",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selects-single",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "The accessible label for the checkbox. For header rows, this defaults to 'Select All'.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "table-head",
    description: "",
    tag: "sp-table-head",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "row",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "table-head-cell",
    description: "",
    tag: "sp-table-head-cell",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "active",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "columnheader",
      },
      {
        name: "sortable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "sort-direction",
        description: "",
        kind: Choice,
        choices: ["asc", "desc"],
        fallback: "",
      },
      {
        name: "sort-key",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "table-row",
    description: "",
    tag: "sp-table-row",
    package: "table",
    slots: [],
    attributes: [
      {
        name: "role",
        description: "",
        kind: Text,
        choices: [],
        fallback: "row",
      },
      {
        name: "selectable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "value",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "tabs",
    description: "",
    tag: "sp-tabs",
    package: "tabs",
    slots: ["", "tab-panel"],
    attributes: [
      {
        name: "auto",
        description: "Whether to activate a tab on keyboard focus or not.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "compact",
        description: "The tab items are displayed closer together.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "direction",
        description: "",
        kind: Choice,
        choices: ["vertical", "vertical-right", "horizontal"],
        fallback: "horizontal",
      },
      {
        name: "emphasized",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "enableTabsScroll",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "quiet",
        description: "The tab list is displayed without a border.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "tabs-overflow",
    description: "",
    tag: "sp-tabs-overflow",
    package: "tabs",
    slots: [],
    attributes: [
      {
        name: "compact",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label-previous",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Scroll to previous tabs",
      },
      {
        name: "label-next",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Scroll to next tabs",
      }
    ],
  },
  {
    directive: "tag",
    description: "",
    tag: "sp-tag",
    package: "tags",
    slots: ["", "avatar", "icon"],
    attributes: [
      {
        name: "deletable",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "disabled",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "readonly",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "tags",
    description: "",
    tag: "sp-tags",
    package: "tags",
    slots: [""],
    attributes: [

    ],
  },
  {
    directive: "textfield",
    description: "",
    tag: "sp-textfield",
    package: "textfield",
    slots: ["help-text", "negative-help-text"],
    attributes: [
      {
        name: "value",
        description: "The value held by the form control",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "allowed-keys",
        description: "A regular expression outlining the keys that will be allowed to update the value of the form control.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "focused",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "invalid",
        description: "Whether the `value` held by the form control is invalid.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "label",
        description: "A string applied via `aria-label` to the form control when a user visible label is not provided.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "name",
        description: "Name of the form control.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "placeholder",
        description: "Text that appears in the form control when it has no value set",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "type",
        description: "",
        kind: Text,
        choices: [],
        fallback: "text",
      },
      {
        name: "pattern",
        description: "Pattern the `value` must match to be valid",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "grows",
        description: "Whether a form control delivered with the `multiline` attribute will change size",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "maxlength",
        description: "Defines the maximum string length that the user can enter",
        kind: Number,
        choices: [],
        fallback: "-1",
      },
      {
        name: "minlength",
        description: "Defines the minimum string length that the user can enter",
        kind: Number,
        choices: [],
        fallback: "-1",
      },
      {
        name: "multiline",
        description: "Whether the form control should accept a value longer than one line",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "readonly",
        description: "Whether a user can interact with the value of the form control",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "tooltip-placement",
        description: "Placement of the tooltip shown when the value is truncated (e.g. 'bottom', 'top').",
        kind: Text,
        choices: [],
        fallback: "bottom",
      },
      {
        name: "rows",
        description: "The specific number of rows the form control should provide in the user interface",
        kind: Number,
        choices: [],
        fallback: "-1",
      },
      {
        name: "valid",
        description: "Whether the `value` held by the form control is valid.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "quiet",
        description: "Whether to display the form control with no visible background",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "required",
        description: "Whether the form control will be found to be invalid when it holds no `value`",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "autocomplete",
        description: "What form of assistance should be provided when attempting to supply a value to the form control",
        kind: Choice,
        choices: ["list", "none", "autocomplete", "autocomplete"],
        fallback: "",
      }
    ],
  },
  {
    directive: "theme",
    description: "",
    tag: "sp-theme",
    package: "theme",
    slots: [""],
    attributes: [
      {
        name: "color",
        description: "The Spectrum color stops to apply to content scoped by this `sp-theme` element.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "scale",
        description: "The Spectrum platform scale to apply to content scoped by this `sp-theme` element.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "lang",
        description: "The language of the content scoped to this `sp-theme` element, see: <a href=\"https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang\" target=\"_blank\">MDN reference</a>.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "system",
        description: "The Spectrum system that is applied to the content scoped to this `sp-theme` element.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "thumbnail",
    description: "",
    tag: "sp-thumbnail",
    package: "thumbnail",
    slots: ["image"],
    attributes: [
      {
        name: "background",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "cover",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "layer",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "size",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "toast",
    description: "",
    tag: "sp-toast",
    package: "toast",
    slots: ["", "action"],
    attributes: [
      {
        name: "open",
        description: "The `open` property indicates whether the toast is visible or hidden.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "timeout",
        description: "When a timeout is provided, it represents the number of milliseconds from when",
        kind: Text,
        choices: [],
        fallback: "null",
      },
      {
        name: "variant",
        description: "The variant applies specific styling when set to `negative`, `positive`, `info`, `error`, or `warning`.",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "icon-label",
        description: "The `iconLabel` property is used to set the `label` attribute on the icon element. This is used to provide a text alternative for the icon to ensure accessibility.",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "tooltip",
    description: "",
    tag: "sp-tooltip",
    package: "tooltip",
    slots: ["icon", ""],
    attributes: [
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "delayed",
        description: "A Tooltip that is `delayed` will its Overlay wait until a warm-up period of",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "disabled",
        description: "Whether to prevent a self-managed Tooltip from responding to user input.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "self-managed",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "offset",
        description: "",
        kind: Number,
        choices: [],
        fallback: "0",
      },
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "tipPadding",
        description: "",
        kind: Number,
        choices: [],
        fallback: "",
      },
      {
        name: "variant",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "tooltip-openable",
    description: "",
    tag: "sp-tooltip-openable",
    package: "tooltip",
    slots: [],
    attributes: [
      {
        name: "open",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "top-nav",
    description: "",
    tag: "sp-top-nav",
    package: "top-nav",
    slots: [""],
    attributes: [
      {
        name: "compact",
        description: "The collection of tabs take up less space",
        kind: Flag,
        choices: [],
        fallback: "",
      },
      {
        name: "label",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "ignore-url-parts",
        description: "A space separated list of part of the URL to ignore when matching",
        kind: Text,
        choices: [],
        fallback: "",
      },
      {
        name: "selectionIndicatorStyle",
        description: "",
        kind: Text,
        choices: [],
        fallback: "noSelectionStyle",
      },
      {
        name: "quiet",
        description: "The Top Nav is displayed without a border.",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "selected",
        description: "",
        kind: Text,
        choices: [],
        fallback: "",
      }
    ],
  },
  {
    directive: "top-nav-item",
    description: "",
    tag: "sp-top-nav-item",
    package: "top-nav",
    slots: [""],
    attributes: [
      {
        name: "selected",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "tray",
    description: "",
    tag: "sp-tray",
    package: "tray",
    slots: [""],
    attributes: [
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      },
      {
        name: "has-keyboard-dismiss",
        description: "When set, prevents the tray from rendering visually-hidden dismiss helpers.",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
  {
    directive: "truncated",
    description: "",
    tag: "sp-truncated",
    package: "truncated",
    slots: [],
    attributes: [
      {
        name: "placement",
        description: "",
        kind: Text,
        choices: [],
        fallback: "top-start",
      },
      {
        name: "success-message",
        description: "",
        kind: Text,
        choices: [],
        fallback: "Copied to clipboard",
      }
    ],
  },
  {
    directive: "underlay",
    description: "",
    tag: "sp-underlay",
    package: "underlay",
    slots: [],
    attributes: [
      {
        name: "open",
        description: "",
        kind: Flag,
        choices: [],
        fallback: "false",
      }
    ],
  },
]
