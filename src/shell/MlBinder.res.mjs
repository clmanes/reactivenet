...
import * as PythonRunner from "./PythonRunner.res.mjs";
import * as ReactiveStore from "./ReactiveStore.res.mjs";
import * as CollectionStore from "./CollectionStore.res.mjs";

let install = (function (read, write, markDerived, storeGet, storeSubscribe, runPython, isEnabled, enable) {
...
async function isEnabled() {
  let match = await Idb.get(enabledKey);
  return match === "yes";
}

function enable() {
  return Idb.set(enabledKey, "yes");
}

let binder = install(CollectionStore.read, CollectionStore.write, DerivedPaths.mark, ReactiveStore.get, ReactiveStore.subscribe, PythonRunner.run, isEnabled, enable);
