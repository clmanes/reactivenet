...
let install = (function (read, loadMakeTable, loadReplaceRows, loadMakeViewer, loadSaveConfig, loadDeleteViewer) {
  const PLUGINS = {
    datagrid: "Datagrid",
    bar: "Y Bar",
    line: "Y Line",
...
function loadMakeTable() {
  let pending = loaded.contents;
  if (pending !== undefined) {
    return pending;
  }
  let pending$1 = import("./ExploreImpl.res.mjs").then(m => m.makeTable);
  loaded.contents = pending$1;
  return pending$1;
}

let binder = install(CollectionStore.read, loadMakeTable, () => import("./ExploreImpl.res.mjs").then(m => m.replaceRows), () => import("./ExploreImpl.res.mjs").then(m => m.makeViewer), () => import("./ExploreImpl.res.mjs").then(m => m.saveConfig), () => import("./ExploreImpl.res.mjs").then(m => m.deleteViewer));

function bind(container, app) {
  binder(container, app);
}
