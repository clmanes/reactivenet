...
let install = (function (run, stopRun, read, write, makeId, now, storeGet, storeSubscribe) {
...
let runner = install(PythonRunner.runWatched, PythonRunner.stop, CollectionStore.read, CollectionStore.write, RecordId.make, Clock.timestamp, ReactiveStore.get, ReactiveStore.subscribe);

function bind(container, app, labels) {
  runner(container, app, labels);
}
