...
import * as Clock from "./Clock.res.mjs";
import * as Draft from "../core/Draft.res.mjs";
...
import * as TimeGrid from "../core/TimeGrid.res.mjs";
import * as Aggregate from "../core/Aggregate.res.mjs";
import * as FileStore from "./FileStore.res.mjs";
import * as MonthGrid from "../core/MonthGrid.res.mjs";
import * as Collection from "../core/Collection.res.mjs";
import * as RowTemplate from "../core/RowTemplate.res.mjs";
...
let binder = install({
  ...
  fill: (template, record, locale, resolve) => RowTemplate.fill(template, {
    id: record.id,
    fields: record.fields.map(field => ({
      name: field.name,
      value: Clock.localize(field.value, locale)
    }))
  }, undefined, (path, id, label) => Stdlib_Option.map(resolve(path, id, label), value => Clock.localize(value, locale))),
  timestamp: Clock.timestamp,
  aggregate: (name, collection, field, decimals) => Stdlib_Option.flatMap(Aggregate.parse(name), kind => Aggregate.compute(kind, collection, field, decimals)),
  stamp: Stamps.apply,
  createdOf: Stamps.createdOf,
  arrange: (records, fields, query, filters, sort, dir, limit) => RowView.limit(RowView.sort(RowView.search(RowView.filterAll(records, filters), query, fields), sort, RowView.directionOf(dir)), limit),
  choices: RowView.values,
  grid: TimeGrid.build,
  blocks: TimeGrid.blocks,
  refusal: TimeGrid.refusal,
  warned: TimeGrid.warned,
  pinned: TimeGrid.pinned,
  ...
  shiftDays: MonthGrid.shiftDays,
  monthStep: MonthGrid.shift,
  dayOf: MonthGrid.dayOf,
  covers: MonthGrid.covers,
  monthOf: MonthGrid.monthOf,
  monthLabel: Clock.monthLabel,
  weekdayNames: Clock.weekdayNames,
  paginate: RowView.page,
  pageCount: RowView.pageCount,
  group: RowView.groups,
  valueOf: RowView.valueOf,
  localize: Clock.localize,
  reading: Draft.reading,
  blank: Draft.blank,
  referenced: RowTemplate.referenced,
  localFile: FileStore.load,
  validate: (controls, locale) => {
    let tag = Stdlib_Option.getOr(Locale.parse(locale), Locale.fallback);
    let withLimit = (key, limit) => Translations.translate(tag, key).replace("{n}", limit);
    return Draft.check(controls).map(complaint => {
      let limit = complaint.problem;
      let message;
      if (typeof limit !== "object") {
        switch (limit) {
          case "Missing" :
            message = Translations.translate(tag, "FieldRequired");
            break;
          case "NotANumber" :
            message = Translations.translate(tag, "FieldNotANumber");
            break;
          case "NotADate" :
            message = Translations.translate(tag, "FieldNotADate");
            break;
          case "NotATime" :
            message = Translations.translate(tag, "FieldNotATime");
            break;
          case "NotAnEmail" :
            message = Translations.translate(tag, "FieldNotAnEmail");
            break;
          case "NotAUrl" :
            message = Translations.translate(tag, "FieldNotAUrl");
            break;
        }
      } else {
        switch (limit.TAG) {
          case "Below" :
            message = withLimit("FieldBelow", limit._0);
            break;
          case "Above" :
            message = withLimit("FieldAbove", limit._0);
            break;
          case "NotMatching" :
            let said = limit._0;
            message = said === "" ? Translations.translate(tag, "FieldNotMatching") : said;
            break;
        }
      }
      return {
        field: complaint.field,
        message: complaint.label + " — " + message
      };
    });
  }
});

function bind(container, app, labels, locale, confirm) {
